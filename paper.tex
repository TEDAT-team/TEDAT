...
\textit{Second}, existing techniques mainly focus on the availability of network reachability, but ignore that users not only have diverse demands on bandwidth,  but also ask for different levels of reliability. Providing reliable bandwidth can be a value-added service for many cloud providers, typically in the form of 
...
For example, Microsoft Azure guarantees its customers at least 99.9\% availability for backup service and 99.95\% availability for ExpressRoute service \cite{azure}.
...
\textit{Third}, 
such heterogeneities and competitions are also not considered by current failure recovery approaches, especially those who allocate bandwidth aggressively \cite{Teavar,swan}, since they may run into congestions when rerouting traffic under network failures. 
...
\textbf{Availability directly translates into profit.}
...
99.0\%, and provides 30\% refund for anything below 99.0\% availability \cite{azure};
Offering high service availability under network failures to retain a good profit is a big challenge.
...
In recent years, there has been a rapid increase in deploying realtime services (video streaming, online game, instant messaging, live broadcast, etc.) over clouds.
