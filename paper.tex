...
We show that state-of-the-art traffic engineering schemes fail to meet the heterogeneous bandwidth  availability demands, especially under diverse link failure probabilities that may vary by several orders of magnitude (see $\S$\ref{background}). We note that, although the general concept of bandwidth-based availability has been considered in some TE works \cite{Google BwE, B4-after}, their methodologies and evaluations are actually targeting only a soft guarantee, i.e., the ratio of the allocated bandwidth to the negotiated one, while we will provide a hard guarantee, i.e., the negotiated bandwidth must be met.
...
instant messaging, live broadcast, etc.) are deployed on the Internet, providing hard guarantee of high service availability under network failures to retain a good profit is a big challenge.
