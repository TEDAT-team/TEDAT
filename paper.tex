...
$v_t^{\mathbf{z}}$ &\multicolumn{6}{|l}{whether tunnel $t$ is available under scenario $\mathbf{z}$}\\
 \hline
  $L_e^{ \mathbf{z}}$ &\multicolumn{6}{|l}{whether link $e$ is available under scenario $ \mathbf{z}$}\\
...
$ T'_k=T'_k \setminus t$;\\
 ...
 $b_{d}^k = b_{d}^k- f_{d}^t;$\\
  $c_t= c_t- f_{d}^t;$\\
  \textit{Update remaining link capacity} ;\\
}
%\For{$t\in T_k$}{
...
remaining capacity and availability has a higher priority (line 7-14). 
After this, if the availability target cannot be roughly satisfied, we will give up  (line 15-16), 
...
Let $L_e^{ \mathbf{z}}$ denote whether link $e$ is available under scenario $ \mathbf{z}$.
...
\sum_{d\in \hat{D}}\sum_{k\in K:t\in T^{ \mathbf{z}}_k} f^{t}_{d}u_t^e \le c_e \times L_e^{ \mathbf{z}}
\end{eqnarray}
\end{small}
...
\KwOut{ $\{f^t_{d}\},F$}
Sort $d\in \hat{D}$ in non-decreasing order with $\frac{w_d}{\sum_{k\in K}\mathbf{b}^k_d}$;\\
...
% \end{algorithm}
 ...
 Let $F$ denote the set that BA demand set that derive full profit.
Firstly, it sorts all the accepted demands  $d\in \hat{D}$ in non-decreasing order according to the ratio of weight to aggregate bandwidth demands, where the aggregate bandwidth demands are derived as ${\sum_{k\in K}\mathbf{b}^k_d}$ (Line 1).
The ordered sequence guarantees demands that have large profit but small bandwidth deriving high priority.
For each admitted demand, the algorithm tries to allocate resource it with remaining network capacity (Line 6-10).
If the network is able to support it, then add to $F$.
If the network is unable to support current demand but  current demand has more profit than the total profit of previous ones,  the algorithm will try to recycle total resources and test that if allocating total network resources can support current demand  (Line 13-17).
If this is true, then algorithm will prefer current demand  (Line13-17), otherwise, the algorithm finishes the iteration (Line 18-19).
%\begin{equation}
