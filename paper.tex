...
a \textit{BA} demand $(b_d, \beta_d, t_d^s, t_d^e)$ means that it requests bandwidth $b_d$ for a life duration between $ t_d^s$ and $t_d^e$, and should be guaranteed at least $\beta_d\%$ of the duration, subjected to the network capacity and possible failures. Such a demand is typically represented by a Service Level Agreement (see Table \ref{target} for real world examples), and different users or applications may pose different demands. 
...
%&\multicolumn{2}{|c|}{$< 99\%$}&\multicolumn{2}{|c|}{25\%}& \\
%&\multicolumn{2}{|c|}{$< 95\%$}&\multicolumn{2}{|c|}{100\%}& \\
\multirow{3}{*}{$(\mathbf{b}_d, \beta_d)$}&\multicolumn{6}{|l}{a BA demand, requiring bandwidth $\mathbf{b}_d$ with }\\
...
%&\multicolumn{6}{|l}{of bandwidth demands over all s-d pairs\footnote{Here we omit the start and end time of this demand, but they will be implicitly considered in our online admission and traffic scheduling.}} \\
A  bandwidth availability demand can be presented as $(\mathbf{b}_d, \beta_d,t_d^s,t_d^e)$,
...
\textbf{Broker} takes cares of the corresponding data center. It consists of  three modules:
%(1) Demand  Aggregator.
