...
Our evaluation demonstrates that, compared with the optimal admission strategy, $\mathsf{BATE}$ can speed up the online admission control by 30$\times$ at the expense of only a xx\% false rejections. 
...
Every second, we emulate link failures by shutting down the corresponding VM interfaces 
...
\textbf{User demands arrive continuously.}
...
Each experiment lasts 100 minutes, where link failures occur probabilistically, 
and is repeated for 50 times.
...
how often its bandwith availability requirement can be guaranteed. 
Since we emulate different link failures according to their probabilities in every second, 
we can measure how the bandwidth a user actually uses deviates from its requirement. 
If such a deviation is less than 1\%, we regard the bandwidth availability as \textit{satisfied in that second}. Figure \ref{e2}(b) shows the overall fraction of satisfication, under 
different levels of availability requirements (i.e., 95\%, 99\% and 99.99\%). 
...
we plot in Figure \ref{e2}(c), for each algorithm, the ratio of the allocated bandwidth 
to the demanded bandwidth. 
The cdf curve shows that FFC is too conservative in bandwidth allocation, 
which failes to allocate proper bandwidth in almost 60\% time.   
...
and achieves a lower satisfication ratio than $\mathsf{BATE}$. 
...
Figure \ref{e2}(d) plots the overall profit $\mathsf{BATE}$, FFC and TEAVAR can achieve for admitted demands, under three different admission control strategies, i.e., fixed, AD (which is the strategy $\mathsf{BATE}$ uses) and optimal. 
...
Figure \ref{e2}(e) further depicts the fraction of profit loss caused by SLA violations, where the baseline for each algorithm is the profit it can achieve when no failures occur. 
\textbf{TBD: $\mathsf{BATE}$ can retain about 15\% more money ratio than FFC and TEAVAR when network fails.}
Compared with TEAVAR, FFC has a lower profit loss ratio due to its conservative bandwidth allocation, as indicated in Figure \ref{e2}(c) earlier. 


\textbf{TBD: We measure total bytes loss according to \textit{iperf} server side reports and packet counters in the switches, then finally derive the data loss ratio.
...
\centering
\subfigure[Link utilization]{
\includegraphics[width=0.23\textwidth]{fig/evaluation/att.pdf}}
\subfigure[Admission time]{
\includegraphics[width=0.23\textwidth]{fig/evaluation/cernet3.pdf}}
