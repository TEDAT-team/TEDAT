...
where the profit loss is only around 10\%.
...
We plot the overall percentage of satisfied BA demands under each arrival rate (averaged across all simulations) in Figure \ref{scheduling-benefit}(a). $\mathsf{BATE}$ nearly always achieves a satisfaction ratio around 100\%, with a leading margin of at least 23\% (with respect to TEAVAR) under a high arrival rate (rate=6 in the figure)\footnote{$\mathsf{BATE}$ leads FFC by around 60\%, which is not shown in the figure.}. 
...
Remeber that our scaling down factor is 5, so these summarized results are all collected under normal network load, where the mean arrival rate in each slot is between 1 and 6. 
