...
Our evaluation demonstrates that, compared with the optimal admission strategy, $\mathsf{BATE}$ can speed up the online admission control by 30 $\times$ at the expense of only a 5\% higher rejection ratio. 
...
\hline
$D, \hat{D}$&\multicolumn{6}{|l}{the set of arrived demands and admitted demands\footnote{When an admitted demand finishes, it will be removed from $\hat{D}$.} }\\
...
In reality, a customer demanding Inter-DC WAN bandwidth resourses could be any application or tenant  spanning multiple DCs, in either a public or a private cloud. 
...
Our abstraction of network failure scenarios and bandwidth availability demands in $\mathsf{BATE}$ are as follows.
...
The Inter-DC WAN is modeled as a directed graph $G(V,E)$, where the set of nodes $V$ represent the DCs, 
and the set of links $V$ represent directed links between them. 
...
%Operators maintain the network empirical data, which can be used to estimate the link risks\cite{jointfailure,overview,evole}.
and we also assume network operators can use historical data to estimate the failure probability $x_i$ for this link. 
...
Then, the expected probability that network scenario  $\mathbf{z}$ will happen \cite{Teavar} is given by 
\begin{eqnarray} \label{state}
%p_{\mathbf{z}}&=&p\left( z_1=z_1',z_2=z_2',z_3=z_3',...,z_{e}=z_{e}'\right)\nonumber \\
p_{\mathbf{z}}&=&\prod_{i=1}^{|E|}(\mathbf{z}_i \times (1-x_i)+ (1-\mathbf{z}_i)(x_i))
\end{eqnarray}
...
where $\mathbf{b}_d$ is a vector $<\mathbf{b}_d^1, \mathbf{b}_d^2, ...>$ of bandwidth demands over all s-d pairs \footnote{Here we omit the start and end time of this demand, but they will be implicitly considered in our online admission and traffic scheduling.}.
...
Our Traffic Engineering uses tunnel-based forwarding \cite{FFC,swan,Teavar}.%where traffic is carried over a set of tunnels.
...
We use $D$ and $\hat{D}$ to represent arrived demands and \textit{admitted} demands, respectively.
Given a demand $d$, our traffic engineering scheme will decide whether to admit it (i.e., $g_d$), 
and the bandwidth that will be allocated for an admitted demand $d \in \hat{D}$ over tunnel $t$ will be $f_d^t$.
Under the above network failure scenario model, we also use 
$v_t^{\mathbf{z}}$ to denote whether tunnel $t$ is available (i.e., $v_t^{\mathbf{z}}=1$)  or not  (i.e., $v_t^{\mathbf{z}}=0$) under the network scenario $\mathbf{z}$. 
Given an allocation result ${f_d^t | \foreach t \in T_k, k \in K}$, 
if (1) the total bandwidth allocated for each s-d pair $k$, i.e., $\sum_{\foreach t \in T_k} f_d^t}$, 
is equal to the bandwidth demand $\mathbf{b}_d^k$ for that pair, and 
(2) for every tunnel $t$ that is used in the allocation (i.e., $\f_d^t>0$), 
...
is no less than the availability demand $\beta_d$, 
then we say this BA demand is satisfied by this exact allocation result in a probabilistic sense. 
If a failure indeed occurs, our traffic engineering scheme will try to reroute traffic that is affected by this failure. If any availability target is violated, a refund will be given back to the customer (see $\S$\ref{background}), and we use $h_d$ to denote the profit (after refunding) for demand $d$.
...
user demands should be admitted as soon as possible, 
while the time needed to exactly sovle this NP-hard problem may be prohibitive, 
...
\begin{enumerate}
    \item When a new demand $d$ arrives, we fix the bandwidth allocation for all admitted demands in $\hat{D}$, then we check whether $d$ can be satisfied by the remaining network capacity and failure  probability. If the answer is positive, then admit $d$ and make a first-time bandwidth allocation for it.
    \item Otherwise, run a greedy algorithm (Algorithm \ref{greedy-1}) to conjecture whether the admitted demands can potentially be rescheduled to accommodate $d$. If the answer is positive, then admit $d$ and make a temporary bandwidth allocation for it, using the remaining network capacity as far as needed \footnote{It's possible that the temporarily allocated bandwidth falls below the demanded bandwidth, but a new allocation strategy satisfying all demands does exist (see Theorem \ref{NP-222}), and will be computed later in our periodical traffic scheduling.}. 
	\item If $d$ still cannot be accomodated, reject the demand.
...
The greedy algorithm tries to conjecture, in an efficient way, whether a allocation strategy satisfying all demands (i.e., including $d$) exists. It works iteratively as follows. In each iteration, 
It finds the demand which has the smallest product of bandwidth target and availability target (i.e., $\sum_{k \in K}\mathbf{b}_d^k \time \beta_d$) at first (line 4). Then if the remaining network capacity cannot satisfiy this demand, we will give up (line 6$sim$8). 
Otherwise, it allocate tunnel bandwidth for this demand, where a tunnel with a smaller product of 
remaining capacity and availability has a higher priority (line 9$sim$13). 
After this, if the availability target can roughly be satisfied, we go for the next iteration, 
otherwise, we also give up (line 14$sim$18).

The time complexity of Algorithm \ref{greedy-1} is $O(|D|*|K|*max(|T_k|))$. 
It is also worth to note that, there is no false positive in conjectures made by Algorithm \ref{greedy-1}, 
...
If a new demand $d$ can be admitted by Algorithm \ref{greedy-1}, 
there must exist an allocation strategy to satisfy the bandwidth availability targets of all demands $D=\hat{D} \cup {d}$.
\end{Theorem} 
%\begin{proof}
...
\begin{eqnarray} \label{Smaller}
 \sum_{t\in T_{k}} f_{d}^t \le \mathbf{b}^k_d, \quad  \forall d \in \hat{D}, k \in K
\end{eqnarray}


$R_{dk}^{\mathbf{z}}$ is the ratio of total reserved bandwidth to demand through pair $k$ for BA demand $d$ under network scenario $\mathbf{z}$ and it is defined as:

 \begin{eqnarray} \label{R2}
R_{dk}^{\mathbf{z}}=\frac{\sum_{t\in T_{k}}f_{d}^tv_t^{\mathbf{z}} }{\mathbf{b}^k_d}, \quad \forall  d \in D, \mathbf{z}\in Z, k \in K.
\end{eqnarray}
...
In reality, tunnel $t$ might be unavailable (i.e., $v_t^{\mathbf{z}} =0$), and if the total reserved bandwidth through all the available tunnels is larger than $\mathbf{b}^k_d, \forall k \in K$,  then bandwidth demand can still be satisfied and network scenario $\mathbf{z}$ can be regarded as \textit{qualified}, i.e., 
...
$\sum_{\forall k: R_{dk}^{\mathbf{z}} \ge 1} p_{\mathbf{z}} \ge \beta_d, \forall d \in \hat{D}$.
...
Let $B_d^{\mathbf{z}}$ denote the lower bound of $R_{dk}^{\mathbf{z}}$ over the $k$ pairs for  $i$ under network scenario $\mathbf{z}$,  i.e.,

 \begin{eqnarray} \label{B2}
B_d^{\mathbf{z}} \le R_{dk}^{\mathbf{z}}, \quad  \forall  d \in \hat{D}, \mathbf{z}\in Z, k \in K
\end{eqnarray}

It is obvious  $B_d^{\mathbf{z}} \le 1$ and $B_d^{\mathbf{z}}=1$ presents scenario $\mathbf{z}$ is qualified, therefore, 
we can define $B_d^{\mathbf{z}}\times p_{\mathbf{z}}$  as the potential achieved availability of $i$ under network scenario  $\mathbf{z}$.
...
\begin{aligned} \label{BATE-TE}
 &maximize  \sum_{i\in I_a} \sum_{\mathbf{z}\in Z}B_d^{\mathbf{z}}\times p_{\mathbf{z}}\\
...
If the total reserved bandwidth through all the available tunnels is larger than $\mathbf{b}^k_d, \forall k \in K$,  then bandwidth demand can be satisfied even tunnel $t$ fails and network scenario $\mathbf{z}$ can be regarded as \textit{safe} .
...
failure recovery problem and consider a special case, in which $\mu_i=0,\forall d \in \hat{D}$.
