...
For a given network scenario $ \mathbf{z}$ (where one link failure occurs) in consideration, 
%let
...
the ration of allocated bandwidth to a user's demanded bandwidth is \footnote{This is the same as equation (3), but we omit the superscript $\mathbf{z}$ of $R_{dk}^\mathbf{z}$.}
\begin{small}
...
We use $r_d$ denote the profit of demand $d$ with rufunding, such that
...
r_d=
\begin{cases}
g_d &\text{$\forall k \in K: R_{dk} \ge 1 $}\\
...
then we have 
\begin{equation}
f^{t}_{d} \ge 0, \quad\forall d \in \hat{D} ,  k\in K,  t\in T^{\mathbf{z}}_k.
\label{constraint-f3}
\end{equation}
and 
\begin{small}
\begin{eqnarray} \label{B-constraint}
\sum_{d\in \hat{D}}\sum_{k\in K, t\in T^{ \mathbf{z}}_k} f^{t}_{d}u_t^e \le c_e \times w_e^{ \mathbf{z}}, \forall e \in E
\end{eqnarray}
\end{small}
...
This mixed-integer linear programming  (MILP) problem can be proved to be NP-hard, 
