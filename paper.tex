...
For an s-d pair $k$ of BA demand $d$, we use $R_{dk}^{\mathbf{z}}$ to denote the ratio of the effective bandwidth to the demanded bandwidth under network scenario $\mathbf{z}$, which is defined as:
...
TEAVAR \cite{Teavar} prunes a scenario if its probability is smaller than a threshold. However, such a threshold is difficult to choose, and it still has to enumerate all 
possible scenarios. Instead, we use a much faster pruning method, 
