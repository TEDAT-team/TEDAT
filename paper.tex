...
On the testbed, we also implement two TE algorithms, i.e., FFC \cite{FFC} and TEAVAR \cite{Teavar}. 
For simulation, we implement more algorithms, including xxx. 
All codes for our implementation and evaluation will be published, 
so that interested readers can use them in other environments or make further improvements. 
...
Compared with the optimal solutions, (i) our admission control algorithm can speed up the admission procedure by 30$\times$ at the expense of only 5\% higher rejection ratio, 
(ii) our pruning-augmented scheduling algorithm runs xx$\times$ faster with only 6\% waste of total bandwidth and 8\% loss in the overall availability, 
and (iii) our greedy failure recovery algorithm needs xx$\times$ less time to react, 
where the achieved profit decreases by less than 10\%.
...
\textbf{Testbed setup}
We build a small testbed with 6 servers to emulate an inter-DC WAN connecting 6 DCs, as shown in Figure \ref{testbed}. 
