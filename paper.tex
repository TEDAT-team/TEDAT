...
$\hat{D}$ is the total \textit{admitted} demand set.
Let $v_t^{\mathbf{z}}$ denote whether tunnel $t$ is available (i.e., $v_t^{\mathbf{z}}=1$)  or not  (i.e., $v_t^{\mathbf{z}}=0$) under network scenario $\mathbf{z}$.
