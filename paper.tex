...
Figure \ref{e2}(d) plots the overall profit $\mathsf{BATE}$, FFC and TEAVAR can achieve for admitted demands, under three different admission control strategies, i.e., fixed, BATE-AD (which is the strategy $\mathsf{BATE}$ uses) and optimal. 
...
In addition to FFC and TEAVAR, the TE algorithms we compare in simulations also include 
SWAN \cite{swan}, SMORE \cite{SMORE} and B4 \cite{B4}. 
