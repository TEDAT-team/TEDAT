...
\caption{Benefits of admission control.}
...
Figure \ref{availability_fig1}(a)  shows $\mathsf{TEDAT}$ has only about 3\% performance gap from the optimal solution and can accept about 50\% more demands than the \textit{Fixed} consistently.
...
If a demand's achieved availability is higher than its availability target throughout duration time slots, then the availability defined in SLA is \textit{satisfication}.
Figure \ref{availability_fig1}(b) demonstrates that $\mathsf{TEDAT}$ can meet the availability SLAs of 40\% more demands than other traffic engineer algorithms.
The violation of SLA for $\mathsf{TEDAT}$ comes from demands rejection.
...
To show this benefit, we adopt \textit{Fixed} admission control for each traffic scheduling algorithm and Figure \ref{availability_fig1}(c) shows the comparisons.
We can see that $\mathsf{TEDAT}$ performs at least 10\% better than other algorithms even under the same admission method.
