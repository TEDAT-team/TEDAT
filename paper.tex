...
Then the expected availability of $\mathbf{z}$ is $p_\mathbf{z}=p_{\{1,1,0,1\}} =0.96\times0.999999\times0.001\times0.999999\simeq 0.000959998$.
...
and the bandwidth that will be allocated for an admitted demand $d \in \hat{D}$ 
...
\textbf{for each s-d pair} $k$, 
if the total allocated bandwidth on available tunnels under $\mathbf{z}$, 
i.e., $\sum_{\forall t \in T_k} f_d^t v_t^{\mathbf{z}}$,  
...
It finds the demand which has the smallest product of bandwidth target and availability target (i.e., $\sum_{k \in K}\mathbf{b}_d^k \times \beta_d$) at first (line 2). Then if the remaining network capacity cannot satisfy this demand, we will give up (line 4-5). 
...
otherwise, we also give up (line 14-15).
