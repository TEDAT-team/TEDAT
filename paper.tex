...
\caption{A simple example with three parallel demands.}
\label{e1}
\end{figure}
...
\caption{Traffic scheduling results under simulations}
...
We plot the overall percentage of satisfied BA demands under each arrival rate (averaged across all simulations) in Figure \ref{scheduling-benefit}(a). $\mathsf{BATE}$ nearly always achieves a satisfaction ratio around 100\%, with a leading margin of up to 40\%, especially under a high arrival rate. 
This clearly demonstrates $\mathsf{BATE}$'s advantage by matching  stringent availability requirements  
with reliable links. 
