...
\begin{equation}
f^{\bm{z}}_{ikt} \ge 0, \quad\forall d \in D ,  k\in K,   t\in T^{\bm{z}}_k.
\label{constraint-f3}
\end{equation}
...
\sum_{i\in I_a}\sum_{k\in K}\sum_{t\in T^{ \bm{z}}_k} f^{ \bm{z}}_{ikt}u_t^e \le c_e \times A_e^{ \bm{z}}
\end{eqnarray}
\end{small}
...
R_{ik}=\frac{\sum_{t\in T^{ \bm{z}}_k} f^{ \bm{z}}_{ikt}}{\mathbf{b}^k_d}, \forall  d \in D_a,  k \in K
\end{eqnarray}
\end{small}
...
\begin{equation}
\begin{cases}
h_i=y_i+\mu_{\gamma_i}*(1-y_i),&{\forall d \in D_a.} \\
R_{ik} < M*y_i+1-y_i,&{\forall d \in D_a,  k\in K.} \\
R_{ik} >= y_i,&{\forall d \in D_a,  k\in K.} \\
y_i \in\{0,1\},&{\forall d \in D_a.} \\
\end{cases}
\label{A-SSS2}
\end{equation}
