...
We plot the overall percentage of satisfied BA demands under each arrival rate (averaged across all simulations) in Figure \ref{scheduling-benefit}(a). $\mathsf{BATE}$ nearly always achieves a satisfaction ratio around 100\%, with a leading margin of at least 20\% under a high arrival rate (rate=6 in the figure). 
...
still performs at least 10\% better than the others. 
...
\end{array}
\end{equation}
where $M$ is an constant larger than the upper bound of $R_{dk}^{\mathbf{z}}$.
...
We transform the all-or-nothing multi-commodity flow problem to this case as follows:
\textbf{TBD:
