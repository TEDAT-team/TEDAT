...
Our evaluation demonstrates that, compared with the optimal admission strategy, $\mathsf{BATE}$ can speed up the online admission control by 30$\times$ at the expense of less than 4\% false rejections. 
...
\begin{figure}
\begin{center}
\includegraphics [width=0.8\columnwidth] {fig/inter-DC-WAN-V3.pdf}
...
%\end{equation}
 ...
 \begin{figure}
\begin{center}
\includegraphics [width=0.75\columnwidth] {fig/testbed.pdf}
...
\includegraphics[width=0.23\textwidth]{fig/evaluation/testbed-admit.pdf}}
\subfigure[Bandwidth availability]{
\includegraphics[width=0.23\textwidth]{fig/evaluation/fake-testbed-target.pdf}}
\subfigure[Profit loss after failures]{
\includegraphics[width=0.23\textwidth]{fig/evaluation/testbet-loss2.pdf}}
\subfigure[Overall profit gain]{
\includegraphics[width=0.23\textwidth]{fig/evaluation/overall-profit.pdf}}
\caption{Demands arrive with poisson process and their duration are with exponential distribution.}
\label{e2}
\end{figure*}
...
(1) $\mathsf{BATE}$ consistently outperforms latests TE algorithms under various topologies, traffic matrices and failure scenarios. With $\mathsf{BATE}$, up to 40\% more bandwidth availability demands can be successfully fulfilled, and 30\% more profit can be retained when failures occur.
...
(3)  $\mathsf{BATE}$ has stable performance: it doesn't depend on specific network topology, routing schemes, etc.
...
Due to space limitations, more details of the evaluations, including the the ratio of the
...
\caption{ An simple example with three user demands whose details are shown in Table \ref{testbed-1}.}
\label{e1}
\end{figure}
...
%\end{tabular}
%\vfill
\subfigure[Rejection percentage]{
\includegraphics[width=0.32\textwidth]{fig/evaluation/sim-admit.pdf}}
\subfigure[Link utilization]{
\includegraphics[width=0.32\textwidth]{fig/evaluation/link-utilization.pdf}}
\subfigure[Admission time]{
\includegraphics[width=0.32\textwidth]{fig/evaluation/admission-control-time.pdf}}
\caption{$\mathsf{BATE}$  vs. Fixed admission control under different network topologies.}
\label{availability_fig1}
\end{figure*}
...
\begin{figure}
\centering
\subfigure[No extra admission control]{
\includegraphics[width=0.45\textwidth]{fig/evaluation/fixed-shuduling.pdf}}
\subfigure[Fixed admission control]{
\includegraphics[width=0.45\textwidth]{fig/evaluation/fixed-scheduling.pdf}}
\caption{Traffic scheduling schemes comparisons.}
...
\begin{figure}[h]
\begin{center}
\includegraphics [width=0.7\columnwidth] {fig/evaluation/sim-monney.pdf}
...
As shown in Figure \ref{availability_fig1}(c), the admission control time of $\mathsf{BATE}$ is less than 1s with about less than 5\% error, which is at least 30$\times$ faster the global optimal solution.
...
Figure \ref{sim-profit} shows the average monetary gain percentage for the scenario in which one failure could occur in the network.
...
Traffic scheduling algorithm cuts off the scenarios that can hardly happen.
...
\If {Recycling network resource allocated to demands in $F$ is able to support $d$}{
Recycle network resource allocated to demands in $F$;\\
     $F= \{d\}$;\\
...
As shown in Figure \ref{e2}(d), there is almost no loss in FFC, since it keeps a low network utilization. 
$\mathsf{BATE}$ has a slight loss caused by , while TEAVAR has the highest loss. 
\end{appendices}
