...
A \textit{one-size-fit-all} approach \cite{Teavar} ignoring these heterogeneities cannot support such SLAs well, and and may even hurt critical and uninterruptible applications when there are competitions on bandwidth. 
...
a BA demand $(d, \beta, \alpha, \gamma)$ means that it requests bandwidth $d$ for a life duration between $\alpha$ and $\gamma$, and should be ensured in at least a fraction $\beta$ of the duration, subjected to the network capacity and possible failures. Such a demand is typically represented by a Service Level Agreement (see Table \ref{target} for real world examples), and different users or applications may pose different demands. 
...
We show that state-of-the-art traffic engineering schemes fail to the heterogeneous bandwidth-based availability demands in an economic manner, especially under diverse link failure probabilities that may vary by several orders of magnitude (see $\S$\ref{background}).
...
$\mathsf{TEDAT}$ strikes balance between efficiency and optimality, so that new demands can be admitted and guaranteed as much as possible with negligible delay.
...
Then based on a linear programming model, our traffic scheduling algorithm sets up tunnels and allocates bandwidth for the admitted demand.
...
At last, based on a mixed-integer linear programming model, the failure recovery procedure pre-computes backup bandwidth allocations and reroute traffic to minimize the revenue loss due to SLA violations, and is proved to be 2-optimal. 
...
we implement $\mathsf{TEDAT}$ as a real system, including a centralized controller and multiple brokers (one for each DC),  together with end-host clients (see $\S$\ref{system}).
 We conduct extensive experiments using real trace driven simulations as well as  on a small testbed  (see $\S$\ref{evaluation}).
